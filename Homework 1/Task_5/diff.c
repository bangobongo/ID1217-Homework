...
{
    // extracting the data in the arguments
    ...
    int line = 0;
    int nr_lines_to_process = (end_line-start_line);

    while (line < nr_lines_to_process)
    {
        int lines_not_equal = memcmp(file1_partition+(line*line_size), 
                                    file2_partition+(line*line_size), 
                                    line_size) != 0;
        if (lines_not_equal)
        {
...
{
    // declare attribute like matrixSum task, not really needed but did it anyway
    pthread_attr_t attr;
    ...
    // max possible memory needed to store a file
    ...
    // most of these are used to make the printing better
    int shortest_file_length, longest_file_length, shortest_file, longest_file;
    ...
    // how many threads to use for comparing lines
    int compare_threads;

    /* 
        number of threads to use for compare cant be 
        higher than amount of rows to process
    */
    if(shortest_file_length<MAXWORKERS)
    {
        compare_threads = shortest_file_length;
    }
    else
    {
        compare_threads = MAXWORKERS;
    }

    // array to track different and similar lines in the files
    int *valid_lines = malloc(shortest_file_length * sizeof(int));

    // Fixed bug of shared memory addresses
    line_compare_data data[compare_threads];
    ...
    */ 
    ...
    { 
        ...
        int start_line = thread_id*(shortest_file_length/compare_threads);

        // line where thread should stop processing, inclusive
        int end_line;

        // if this is the last iteration
        if(!(i+1 < compare_threads))
        {
        ...
        else
        {
            ...
            end_line = (thread_id+1)*(shortest_file_length/compare_threads);
        }
